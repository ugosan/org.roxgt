...
import org.roxgt.diagram.part.RoxgtLinkDescriptor;
...
public class GraphCanonicalEditPolicy extends CanonicalConnectionEditPolicy {
	...
	protected List getSemanticChildrenList() {
		...
		for (Iterator it = RoxgtDiagramUpdater.getGraph_79SemanticChildren(
	...
	protected Set getFeaturesToSynchronize() {
		if (myFeaturesToSynchronize == null) {
			...
			myFeaturesToSynchronize
					.add(RoxgtPackage.eINSTANCE.getGraph_Nodes());
		}
	...
	private Collection refreshConnections() {
		...
		for (Iterator linksIterator = existingLinks.iterator(); linksIterator
			...
			int diagramLinkVisualID = RoxgtVisualIDRegistry
					.getVisualID(nextDiagramLink);
			for (Iterator LinkDescriptorsIterator = linkDescriptors.iterator(); LinkDescriptorsIterator
					.hasNext();) {
				RoxgtLinkDescriptor nextLinkDescriptor = (RoxgtLinkDescriptor) LinkDescriptorsIterator
						.next();
	...
	private Collection collectAllLinks(View view, Map domain2NotationMap) {
		...
		case GraphEditPart.VISUAL_ID: {
			...
			result.addAll(RoxgtDiagramUpdater.getGraph_79ContainedLinks(view));
			break;
		}
		case EdgeEditPart.VISUAL_ID: {
			domain2NotationMap.put(view.getElement(), view);
			result.addAll(RoxgtDiagramUpdater.getNode_1001ContainedLinks(view));
			break;
		}
		case PropertyEditPart.VISUAL_ID: {
			domain2NotationMap.put(view.getElement(), view);
			result.addAll(RoxgtDiagramUpdater
					.getProperty_2001ContainedLinks(view));
			break;
		}
		case EdgeTargetEditPart.VISUAL_ID: {
			domain2NotationMap.put(view.getElement(), view);
			result.addAll(RoxgtDiagramUpdater.getEdge_3001ContainedLinks(view));
			break;
		}
		}
	...
	private Collection createConnections(Collection linkDescriptors,
		...
		for (Iterator linkDescriptorsIterator = linkDescriptors.iterator(); linkDescriptorsIterator
				.hasNext();) {
			final RoxgtLinkDescriptor nextLinkDescriptor = (RoxgtLinkDescriptor) linkDescriptorsIterator
					.next();
